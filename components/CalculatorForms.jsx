...
function StampDutyForm({ config }) {
    ...
    const firstHomeEstimate = firstHomeBuyer ? calculateFirstHomeDutyEstimate({ state, price: numericPrice, duty: result.duty, propertyType }) : { duty: result.duty, applied: false, reason: '' };
      ...
      <ResultPanel invalid={invalid} hasInput={numericPrice > 0}>
        ...
        <div className="threshold-table">
          {thresholdRows.map((threshold) => <span key={`${threshold.min}-${threshold.max}`}>
            <span>{new Intl.NumberFormat('en-AU', { style: 'currency', currency: 'AUD', maximumFractionDigits: 0 }).format(threshold.min)} – {threshold.max ? new Intl.NumberFormat('en-AU', { style: 'currency', currency: 'AUD', maximumFractionDigits: 0 }).format(threshold.max) : 'above'}</span><strong>{threshold.rate}%</strong>
          </span>)}
        </div>
      </ResultPanel>
    </>);
