...
export function calculateFirstHomeDutyEstimate({ state, price, duty, propertyType, waRegion = 'metro-peel' }) {
...
export function calculateStampDuty(price, config, state = config.state, propertyType = 'established-home') {
    const stateConfigKey = state === 'ACT' && propertyType === 'investment-property' ? 'ACT_INVESTMENT' : state;
